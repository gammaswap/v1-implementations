...
contract PositionManager is IPositionManager, IAddLiquidityCallback, ERC721 {
    ...
    function addLiquidityCallback(
        ...
        if (amount0Owed > 0) pay(decoded.poolKey.token0, decoded.payer, decoded.destination, amount0Owed);
        if (amount1Owed > 0) pay(decoded.poolKey.token1, decoded.payer, decoded.destination, amount1Owed);
    }
    ...
    // **** ADD LIQUIDITY ****
    function addLiquidity(AddLiquidityParams calldata params) external virtual override ensure(deadline) returns (uint amountA, uint amountB, uint liquidity) {
        //TODO: Can't call addLiquidity from here. So will have to deposit in Uniswap, then add liquidity. We'll use our own router for Uniswap
        params.gammaPool = poolFor(params.tokenA, params.tokenB, params.protocolId);
        (amountA, amountB,,) = IProtocolRouter(protocols[params.protocolId]).addLiquidity(params,
        abi.encode(AddLiquidityCallbackData({poolKey: poolKey, payer: msg.sender, destination: msg.sender})));//TODO: change destination to uniPool
        liquidity = IGammaPool(params.gammaPool).mint(params.to);
    }
