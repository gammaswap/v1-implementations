...
describe("LiquidationStrategy", function () {
  ...
  describe("Test refund functions", function () {
    ...
    it("write  down, payableLiquidity < loanLiquidity", async function () {
      ...
      expect(res2.events[0].event).to.equal("WriteDown");
      expect(res2.events[0].args.writeDownAmt).to.equal(expectedWriteDown);
      expect(res2.events[1].event).to.equal("WriteDown2");
      expect(res2.events[1].args.loanLiquidity).to.equal(payableLiquidity);
  ...
  describe("liquidate with LP", function () {
    ...
    it("partial liquidation, no write down", async function () {
      ...
      for (let i = 0; i < res.events.length; i++) {
        expect(res.events[i].event).to.not.equal("WriteDown");
      }
    ...
    it("partial liquidation, write down", async function () {
      ...
      expect(res.events[0].event).to.equal("WriteDown");
      expect(res.events[0].args.writeDownAmt).to.equal(writeDownAmt);
  ...
  describe("liquidate with collateral", function () {
    ...
    it.skip("does not have enough margin so it liquidates", async function () {
    ...
    it.skip("liquidate with rebalance", async function () {
      const startLiquidity = ONE.mul(40);
      const startLpTokens = ONE.mul(20);
      const loanLiquidity = ONE.mul(20000);
      const loanLPTokens = ONE.mul(10);
