...
contract CalcDeltasBase is Test {
    ...
    function calcSellQty(uint256 reserve0, uint256 reserve1, uint256 amount0) internal view returns(uint256) {
        return (reserve1 * amount0 * 1000 / ((reserve0 - amount0) * 997)) + 1;
    }

    function calcBuyQty(uint256 reserve0, uint256 reserve1, uint256 amount0) internal view returns(uint256) {
        return reserve1 * amount0 * 997 / (reserve0*1000 + amount0);
    }
    ...
    function updateTokenQtys(uint128 tokensHeld0, uint128 tokensHeld1, uint128 reserve0, uint128 reserve1,
        uint256 deltas0, uint256 deltas1) internal view returns(uint128, uint128, uint128, uint128) {
        if(deltas0 > 0) {
            uint256 soldToken = calcSellQty(reserve0, reserve1, deltas0);
            tokensHeld1 -= uint128(soldToken);
            tokensHeld0 += uint128(deltas0);
            reserve1 += uint128(soldToken);
            reserve0 -= uint128(deltas0);
        } else if(deltas1 > 0) {
            uint256 soldToken = calcSellQty(reserve1, reserve0, deltas1);
            tokensHeld0 -= uint128(soldToken);
            tokensHeld1 += uint128(deltas1);
            reserve0 += uint128(soldToken);
            reserve1 -= uint128(deltas1);
        }
        return(tokensHeld0, tokensHeld1, reserve0, reserve1);
    }

    function createRatioParameter(uint128 tokensHeld0, uint128 tokensHeld1, uint8 move) internal view returns(uint256[] memory ratio) {
